...
`optimize-resume` 是一个面向 Codex 的简历优化 Skill。它用于在不修改原简历的前提下，复制一份新的简历文件，只优化文字内容，并在聊天窗口输出一份完整的修改建议。

这个 Skill 的核心思想来自一套偏实战的求职简历方法：让简历更符合 HR 的阅读习惯，把经历写成岗位匹配、结果清晰、数据量化的表达，同时在真实前提下尽可能扩大候选人的呈现边界。
...
`optimize-resume` is a Codex skill for resume optimization. It creates a revised copy of the user's resume, edits wording only, preserves the original file, and returns complete revision advice in the chat.

The skill is based on a practical resume-writing method: make the resume easier for HR to scan, rewrite experience around job fit and measurable outcomes, and present the candidate's real background in the strongest honest way.
